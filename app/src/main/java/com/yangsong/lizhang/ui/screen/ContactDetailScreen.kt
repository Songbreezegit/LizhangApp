...
@Composable
fun ContactDetailScreen(viewModel:ContactDetailViewModel,onBack:()->Unit,onAdd:()->Unit){
    val state by viewModel.uiState.collectAsStateWithLifecycle()
    var filter by remember{mutableStateOf<GiftDirection?>(null)}
    var showFilter by remember{mutableStateOf(false)}
    val snackbar=remember{SnackbarHostState()}
    val scope=rememberCoroutineScope()
    val developing=stringResource(R.string.feature_developing)
    val visibleRecords=remember(state.records,filter){state.records.filter{filter==null||it.direction==filter}}
    Scaffold(
        topBar={AppTopBar(state.contact?.name?:stringResource(R.string.nav_contact_detail),onBack){IconButton({scope.launch{snackbar.showSnackbar(developing)}}){Icon(Icons.Outlined.Edit,stringResource(R.string.contact_edit))}}},
