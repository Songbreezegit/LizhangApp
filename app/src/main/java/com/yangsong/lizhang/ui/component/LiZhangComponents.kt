...
@Composable fun DirectionSelector(selected:GiftDirection,onSelected:(GiftDirection)->Unit){Column(verticalArrangement=Arrangement.spacedBy(8.dp)){Text(stringResource(R.string.field_direction),fontWeight=FontWeight.SemiBold);Row(horizontalArrangement=Arrangement.spacedBy(10.dp)){GiftDirection.entries.forEach{d->val color=if(d==GiftDirection.RECEIVED)CoralContainer else MintContainer;val contentColor=if(d==GiftDirection.RECEIVED)CoralOnContainer else MintOnContainer;FilterChip(d==selected,{onSelected(d)},{Text(stringResource(d.labelRes()))},leadingIcon={Icon(if(d==GiftDirection.RECEIVED)Icons.AutoMirrored.Outlined.CallReceived else Icons.AutoMirrored.Outlined.CallMade,null)},colors=FilterChipDefaults.filterChipColors(selectedContainerColor=color,selectedLabelColor=contentColor,selectedLeadingIconColor=contentColor),border=FilterChipDefaults.filterChipBorder(enabled=true,selected=d==selected,borderColor=Color.Transparent,selectedBorderColor=Color.Transparent))}}}}
@OptIn(ExperimentalLayoutApi::class) @Composable fun EventTypeSelector(selected:EventType,onSelected:(EventType)->Unit){Column(verticalArrangement=Arrangement.spacedBy(8.dp)){Text(stringResource(R.string.field_event),fontWeight=FontWeight.SemiBold);FlowRow(horizontalArrangement=Arrangement.spacedBy(8.dp)){EventType.entries.forEach{type->FilterChip(type==selected,{onSelected(type)},{Text(stringResource(type.labelRes()))},colors=FilterChipDefaults.filterChipColors(selectedContainerColor=ApricotContainer,selectedLabelColor=ApricotOnContainer),border=FilterChipDefaults.filterChipBorder(enabled=true,selected=type==selected,borderColor=Color.Transparent,selectedBorderColor=Color.Transparent))}}}}
