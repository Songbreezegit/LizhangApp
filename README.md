...
“记一笔”已具备可搜索联系人选择器、快速新建联系人、全中文年月日选择器、字段校验、保存中防重复提交和成功反馈。联系人详情支持收到/送出记录筛选。OCR 已建立 `StateFlow` 驱动的介绍、不可用、待确认和成功状态，以及低置信度、疑似重复、逐条编辑、删除和确认弹窗组件；真实识别服务接入前不会读取图片或写入数据。设置、导出和备份入口均提供明确的“功能完善中”反馈，不执行虚假操作。
...
2. 实现完整联系人编辑页面，并补充删除联系人和礼金记录的危险操作确认。
3. 为 ViewModel 与 Compose 状态添加自动化单元测试和截图测试。
