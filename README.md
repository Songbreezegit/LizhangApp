...
“记一笔”主入口先提供“手动添加 / 拍照识别礼簿”两种方式；首页悬浮添加按钮复用相同分流。手动表单具备可搜索联系人选择器、快速新建联系人、全中文年月日选择器、字段校验、保存中防重复提交和成功反馈。首页收礼、送礼和日历快捷入口均为独立页面，不再复用搜索页面；顶部铃铛进入未来 30 天日期提醒。联系人模块支持搜索、排序、新增、详情查看、完整资料编辑和删除；删除联系人会明确提示相关礼金记录将级联删除，删除单条礼金记录也必须二次确认。OCR 已建立 `StateFlow` 驱动的介绍、不可用、待确认和成功状态，以及低置信度、疑似重复、逐条编辑、删除和确认弹窗组件；真实识别服务接入前不会读取图片或写入数据。设置、导出和备份入口均提供明确的“功能完善中”反馈，不执行虚假操作。
