...
全局采用浅暖色与轻玻璃组件，兼容深色主题。底部导航为浮动圆角栏，按钮、筛选、卡片和弹窗共享样式参数。联系人导入大卡片已移除，右下角菜单提供“通讯录导入”和“手动添加”；再次点击、外部点击或返回键会收起，管理模式隐藏菜单。联系人按稳定 ID 懒加载，大字体下净往来移到姓名下方，底部预留浮层空间。

猫咪插画统一使用具有真实 Alpha 通道的 PNG，避免深色页面出现白色底块。提醒日期仍支持当天、提前 1 / 3 / 7 天，选项改为统一弹窗内的单选行。业务规则、Room 结构、备份格式与导航状态策略保持不变。

设计参数、资产来源与验收记录见 [UI 重构说明](docs/UI重构V1.md)。版本为 `versionName = 0.3.0`、`versionCode = 3`。
