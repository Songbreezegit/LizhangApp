...
当前数据库版本为 1。备份恢复使用独立领域格式，不直接复制 SQLite 文件；恢复前校验版本、摘要、外键和 SHA-256 完整性，并在单个 Room 事务中替换数据。
...
- 备份完整字段往返、篡改检测、无效外键和恢复确认。
- 年度提醒、跨月跨年和闰日边界。

四个主页面已建立 Compose Preview Screenshot Testing 基础，并补充首页与联系人空状态大字体、记一笔标准状态、放弃填写确认及深色大字体、“我的”深色模式和隐私说明大字体基准。首页年度汇总卡会根据系统字体比例自适应增高，避免金额摘要被截断；记一笔保存操作在标准字体与大字体下均保持可见。主要修改至少执行：
...
工程基础、核心数据模型、CRUD、导航、统计、搜索、提醒、导出、备份恢复、主题切换和设置说明页面均已完成。OCR 已从代码、导航、资源、依赖、权限、测试和文档中移除，应用不再申请联网权限，也不再读取 AI Studio 令牌。

## 后续开发计划

1. 完成 Pixel 9、物理设备、大字体和深色模式的完整易用性走查。
2. 为备份文件增加可选密码加密和跨数据库版本恢复测试。
