...
当前数据库版本为 1。备份恢复使用独立领域格式，不直接复制 SQLite 文件；普通备份使用 SHA-256 完整性校验，可选密码备份使用 PBKDF2-HMAC-SHA256 与 AES-256-GCM 加密认证，并在单个 Room 事务中恢复数据。旧的未加密备份保持兼容。
...
- 未来 30 天年度日期提醒，支持通知权限、当天、提前 1 天、提前 3 天、提前 7 天及提醒时间设置。
- CSV 与标准 XLSX 导出。
- `.lizhangbackup` 本地备份、校验、确认和事务恢复；支持用户选择普通备份或密码加密备份，密码不会持久化保存。
- Material 3 浅色/深色配色。
...
- 备份完整字段往返、普通与加密格式兼容、错误密码、密文篡改、无效外键和恢复确认。
- 年度提醒、多档提前日期、跨月跨年和闰日边界。

四个主页面已建立 Compose Preview Screenshot Testing 基础，并补充首页与联系人空状态大字体、记一笔标准状态、放弃填写确认及深色大字体、“我的”深色模式、加密备份密码状态和隐私说明大字体基准。首页年度汇总卡会根据系统字体比例自适应增高，避免金额摘要被截断；记一笔保存操作在标准字体与大字体下均保持可见。主要修改至少执行：
...
工程基础、核心数据模型、CRUD、导航、统计、搜索、多档年度提醒、导出、普通与密码加密备份恢复、主题切换和设置说明页面均已完成。OCR 已从代码、导航、资源、依赖、权限、测试和文档中移除，应用不再申请联网权限，也不再读取 AI Studio 令牌。

## 后续开发计划

1. 完成 Pixel 9、物理设备、大字体和深色模式的完整易用性走查。
2. 增加跨数据库版本恢复测试和备份格式迁移演练。
3. 根据真实用户反馈评估农历日期、自定义提醒周期和提醒记录直达能力。
