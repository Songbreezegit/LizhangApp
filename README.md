...
- 联系人搜索、中文姓名排序、新增、编辑、详情与删除；新增入口采用与首页一致的圆形主色悬浮按钮；从联系人详情新增往来会自动预选当前联系人，编辑联系人后未保存返回会先居中确认。
...
工程基础、核心数据模型、CRUD、导航、统计、搜索、多档年度提醒及通知直达详情、导出、v1/v2 兼容的普通与密码加密备份恢复、主题切换和设置说明页面均已完成。基础流程已补齐联系人上下文预选、真实日期上限、失效联系人回退、全局加载重试和系统自动备份禁用策略。Pixel 9 的 1.3 倍字体与深色模式主流程走查已完成，并修复表单可发现性和固定浅色容器对比度。OCR 已从代码、导航、资源、依赖、权限、测试和文档中移除，应用不再申请联网权限，也不再读取 AI Studio 令牌。
