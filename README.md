...
- 未来 30 天年度日期提醒，支持通知权限、当天或提前一天、提醒时间设置。
- CSV 与标准 XLSX 导出。
...
工程基础、核心数据模型、CRUD、导航、统计、搜索、提醒、导出、普通与密码加密备份恢复、主题切换和设置说明页面均已完成。OCR 已从代码、导航、资源、依赖、权限、测试和文档中移除，应用不再申请联网权限，也不再读取 AI Studio 令牌。

## 后续开发计划

1. 完成 Pixel 9、物理设备、大字体和深色模式的完整易用性走查。
2. 增加跨数据库版本恢复测试和备份格式迁移演练。
3. 根据真实用户反馈扩展提醒周期，例如提前三天或农历日期。
