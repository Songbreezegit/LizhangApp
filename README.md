...
礼账是一款面向家庭的本地优先人情礼金记录应用。产品聚焦礼金往来、联系人管理与历史礼簿迁移。正式业务数据保存在本机；当前 Debug 版本正在验证可主动选择的 AI Studio 云端礼簿识别，Release 仍保持离线 OCR。
...
- OpenCV Android 4.12（文档方向、透视与图像增强）
- Retrofit / OkHttp / Moshi（Debug AI Studio 异步 OCR）
...
图片 URI → OpenCV → PP-OCRv6 → 质量评估 → VL/ML Kit 降级 → 校对 → Room 事务导入
```
...
│   ├── mapper/           # 数据实体与领域模型映射
│   ├── image/            # OpenCV 礼簿图像预处理
│   ├── ocr/              # 混合 OCR 策略与 Room 事务导入
│   ├── remote/paddle/    # AI Studio 异步任务与结果映射
...
如需验证 Debug 云端 OCR，在根目录 `local.properties` 配置 `AISTUDIO_ACCESS_TOKEN=临时令牌`。该文件不会提交，但令牌会进入 Debug APK；测试结束后必须废止并更换。Release 构建固定使用空令牌并关闭云端 OCR。详细步骤见 `docs/AIStudio-PaddleOCR调试配置.md`。
...
4. 使用不同纸张、手写体和光照条件建立脱敏 OCR 样本集，持续改进版面解析规则；完成验证后把 Debug 直连迁移到后端代理或端侧模型。
