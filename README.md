...
联系人页面新增“从通讯录导入”：仅在用户主动进入后申请 `READ_CONTACTS`，默认选中新联系人；标准化同号禁止导入，同名不同号标记“同名联系人”且默认不选，可手动选择。支持搜索、全选及脱敏预览。提交时在 Room 事务内复查，只新建、不覆盖或合并，返回后展示成功、已存在跳过、可能重复未选择三项统计并刷新列表。
