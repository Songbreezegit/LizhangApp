...
首页、联系人、联系人详情、记一笔、搜索、统计、OCR 导入入口和“我的”页面均已应用统一视觉系统。底部四项主入口调整为“首页 / 联系人 / 记一笔 / 我的”，统计、搜索和 OCR 作为首页快捷流程进入。首页汇总、最近记录、联系人查询、礼金保存和统计聚合使用现有 Repository 数据。

“记一笔”已具备可搜索联系人选择器、快速新建联系人、全中文年月日选择器、字段校验、保存中防重复提交和成功反馈。联系人模块支持搜索、排序、新增、详情查看、完整资料编辑和删除；删除联系人会明确提示相关礼金记录将级联删除，删除单条礼金记录也必须二次确认。OCR 已建立 `StateFlow` 驱动的介绍、不可用、待确认和成功状态，以及低置信度、疑似重复、逐条编辑、删除和确认弹窗组件；真实识别服务接入前不会读取图片或写入数据。设置、导出和备份入口均提供明确的“功能完善中”反馈，不执行虚假操作。
...
底部导航固定为首页、联系人、记一笔、我的四项；搜索、统计和 OCR 从首页进入，不占用底部导航位置。
...
2. 为 ViewModel 与 Compose 状态添加自动化单元测试和截图测试。
3. 实现礼金记录详情与编辑，保持所有危险操作必须二次确认。
